...
// Auto-Email Brain Phase 3 — cron entrypoint
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const org of orgRows) {
      ...
      // load org name if you have an org table; fallback to id
      ...
      const schedules = await sql`
        SELECT prs.*, v.name AS vendor_name, p.coverage_type, p.expiration_date
      ...
      for (const row of schedules) {
        ...
        // Basic compliance summary (from cache if available)
        ...
        // 4) Plan broker email for tighter windows
        if (stage <= 7) {
          await planAutoEmailForRenewal({
            scheduleRow: row,
            orgName,
        ...
        // Update schedule next_check_at so we don't spam
        await sql`
      ...
      // 5) Process pending queued emails (send)
