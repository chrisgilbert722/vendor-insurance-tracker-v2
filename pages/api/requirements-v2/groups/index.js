...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (method === "POST") {
      ...
      if (!orgId || !name) {
        return res.status(400).json({ ok: false, error: "Missing orgId or name" });
      }
    ...
    if (method === "DELETE") {
      ...
      await client.query("DELETE FROM requirements_groups_v2 WHERE id = $1", [id]);
