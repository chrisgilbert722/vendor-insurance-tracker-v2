...
function computeGroupScore(ruleResults, groupSeverity) {
  ...
  for (const r of ruleResults) {
    ...
    score -= 15 * ruleWeight * groupWeight;
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const vendorId of vendorsToRun) {
      const summary = await runEngineForVendor(
        supabase,
        orgId,
        vendorId,
        groups,
...
/* ============================================================
   CORE ENGINE FOR A SINGLE VENDOR
============================================================ */
async function runEngineForVendor(
  supabase,
  orgId,
  vendorId,
  groups,
  ...
  const policyObj = policies?.length ? buildPolicyObj(policies) : {};
  ...
  autoGeneratedAlerts.push(
    ...checkCoverageLimits(normalized, "standard_construction")
  );
  ...
  for (const rule of rules) {
    ...
    if (rule.type === "coverage" && rule.condition === "exists") {
      passed = !!actual;
    }
  ...
  return {
    ...
    failedRuleCount: results.filter((r) => !r.passed).length,
