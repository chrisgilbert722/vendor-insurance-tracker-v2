...
// Wizard Step 2 — Vendor File Upload (AUTH-AWARE, FAIL-OPEN)
...
export default function VendorsUploadStep({ orgId, onUploadSuccess }) {
  ...
  async function handleUpload() {
    ...
    try {
      // 🔑 GET SUPABASE SESSION (CRITICAL FIX)
      const {
        data: { session },
      } = await supabase.auth.getSession();

      if (!session?.access_token) {
        throw new Error("Session not ready — please try again in 1–2 seconds");
      }
      ...
      // 🔓 FAIL-OPEN — advance even if rows empty
      if (!rows.length) {
        onUploadSuccess?.({
          headers,
          rows: [],
