...
export default function RequirementsV3Page() {
  ...
  async function handleCreateRule() {
    ...
    if (!field_key) return;
    const expected_value = prompt("Expected value (e.g. 1000000 or 'General')");
  ...
  async function handleRunEngine() {
    try {
      ...
      if (json.ok) {
        alert(
          `Engine run complete!\nVendors: ${json.vendors_evaluated}\nRules: ${json.rules_evaluated}\nAlerts Created: ${json.alerts_created}`
        );
      } else {
        alert("Engine error: " + json.error);
      }
    } catch (err) {
