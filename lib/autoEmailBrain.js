...
// Auto-Email Brain (Phase 3) — LIVE RESEND INTEGRATION (BUILD-SAFE EXPORTS)
...
export async function generateRenewalEmail(params) {
  ...
  try {
    ...
    return {
      subject: `Renewal request — ${params.coverage || "Policy"}`,
      body: text,
    };
  }
}
...
/* ============================================================
   PROCESS QUEUE — Send Emails LIVE (Resend via sendEmail())
============================================================ */
...
/* ============================================================
   ✅ REQUIRED EXPORT — planAutoEmailForRenewal
   (Build error fix: this must exist for cron + api routes)
============================================================ */
export async function planAutoEmailForRenewal({
  ...
  let vendorEmailQueued = false;
  let brokerEmailQueued = false;
  ...
  if (vendorEmail) {
    await enqueueRenewalEmail({
      ...
      body: aiEmail.body,
      meta: { source: "autoEmailBrain" },
    });
    vendorEmailQueued = true;
  }
  ...
  if (brokerEmail) {
    ...
    brokerEmailQueued = true;
  }

  return { vendorEmailQueued, brokerEmailQueued };
}
