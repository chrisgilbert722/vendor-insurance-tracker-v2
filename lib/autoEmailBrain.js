...
function buildEmailPrompt({
  ...
  const tone =
    target === "vendor"
      ? "friendly, simple, plain-English"
      : "professional, broker-facing";
...
export async function planAutoEmailForRenewal({
  ...
  if (vendorEmail) {
    await enqueueRenewalEmail({
      ...
      meta: { coverage, expDate, daysLeft },
    });
  }

  // Broker email (only when <= 7 days)
  if (brokerEmail && stage <= 7) {
    await enqueueRenewalEmail({
      orgId: scheduleRow.org_id,
      vendorId: scheduleRow.vendor_id,
      policyId: scheduleRow.policy_id,
  ...
  return {
    vendorEmailSent: !!vendorEmail,
    brokerEmailSent: !!brokerEmail && stage <= 7,
  };
}
/* ============================================================
   HIGH-LEVEL BRAIN — Run All Due Email Jobs Automatically
============================================================ */
export async function autoEmailBrain() {
  // Load all renewal schedule rows that are due to email
  const rows = await sql`
    SELECT 
      s.*, 
