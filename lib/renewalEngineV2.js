...
/**
 * Execute the renewal logic for one policy schedule row.
 */
export async function runRenewalForSchedule(scheduleRow) {
  const stage = computeRenewalStage(scheduleRow);

  // If no stage triggered, schedule next daily check.
  if (!stage) {
    await sql`
  ...
  const vendorRows = await sql`
    SELECT name FROM vendors WHERE id = ${scheduleRow.vendor_id} LIMIT 1;
  `;
...
export async function runRenewalEngineForOrgV2(orgId) {
  ...
  return { orgId, triggered: results.length, details: results };
}
...
export async function runRenewalEngineAllOrgsV2() {
  ...
  const output = [];
  for (const o of orgs) {
    const r = await runRenewalEngineForOrgV2(o.org_id);
    output.push(r);
  }
  return output;
}
